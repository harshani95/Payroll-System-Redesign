...
const EmployeeTable = ({ data }) => {
  ...
  return (
    <div className={`container mx-auto px-4 ${containerBg} p-2 rounded-md`}>
      ...
      <div className="flex items-center space-x-2 mb-2">
        <div className="inline-flex rounded-md overflow-hidden border">
          {["Settings", "Refresh", "Reset", "Excel"].map((btn, i) => (
            <button
              key={i}
              className={`px-3 py-1 text-sm cursor-pointer border-r last:border-r-0 ${
                darkMode
                  ? "text-gray-100 hover:text-blue-400 border-gray-700"
        ...
        <select
          className={`p-1 text-sm ml-auto rounded-md focus:outline-none ${inputBg}`}
        >
          <option>10</option>
          <option>25</option>
          <option>50</option>
              ...
              data.map((row, index) => (
                <tr key={index} className={`${hoverRowBg}`}>
                  {headers.map((header, idx) => (
                    <td key={idx} className="px-4 text-sm">
                      {row[header] || "-"}
                    </td>
                  ))}
                </tr>
